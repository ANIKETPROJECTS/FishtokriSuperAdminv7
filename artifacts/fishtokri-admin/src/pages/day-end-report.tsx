...
function InvoiceModal({ order, onClose }: { order: any; onClose: () => void }) {
  ...
  const handlePrint = async () => {
    const itemRows = items.map((it:any) => {
      ...
      return `<tr><td style="padding:6px 4px;border-bottom:1px solid #eee;font-weight:700;">${it.name}</td><td style="padding:6px 4px;border-bottom:1px solid #eee;text-align:right;">${qty}</td><td style="padding:6px 4px;border-bottom:1px solid #eee;text-align:right;">${(qty*rate).toFixed(2)}</td></tr>`;
    }).join("");
            ...
            <table className="w-full text-[15px]">
              <thead>
                <tr className="border-b border-gray-700 text-left">
                  <th className="py-1.5 font-bold">Item</th><th className="py-1.5 text-right font-bold">Qty (pack)</th><th className="py-1.5 text-right font-bold">Amount</th>
                </tr>
              </thead>
              <tbody>
                {items.map((it:any,i:number)=>{const qty=Number(it.quantity)||1,rate=Number(it.price)||0;return(<tr key={i}><td className="py-1.5 font-bold">{it.name}</td><td className="py-1.5 text-right">{qty}</td><td className="py-1.5 text-right">{(qty*rate).toFixed(2)}</td></tr>);})}
