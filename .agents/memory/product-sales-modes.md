...
**How to apply:** Keep new product forms, imports, API validation, menu filtering, and documentation aligned to these two values. Treat any legacy combined-mode record as normal rather than offering or creating it. When a preorder delivery date changes, filter preorder products using the saved availability schedule and remove cart items that are no longer eligible.